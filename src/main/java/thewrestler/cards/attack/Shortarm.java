...
public class Shortarm extends CustomCard {
  ...
  public Shortarm() {
    super(ID, NAME, getCardResourcePath(IMG_PATH), COST, getDescription(getEnhancedSprainAmount(SPRAIN_AMOUNT)), TYPE,
  ...
  public void upgrade() {
    if (!this.upgraded) {
      ...
      this.rawDescription = getDescription(getEnhancedSprainAmount(this.magicNumber));
      initializeDescription();
    }
  }

  private static int getEnhancedSprainAmount(int baseAmount) {
    return 2 * baseAmount;
  }

  private static String getDescription(int upgradedSprainAmount) {
    return DESCRIPTION + upgradedSprainAmount + EXTENDED_DESCRIPTION[0];
  }
