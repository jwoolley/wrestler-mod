...
import thewrestler.patches.relics.BottledWrestlerField;
...
public class BottledWrestlerRelic extends CustomRelic implements CustomBottleRelic, CustomSavable<Integer> {
    ...
    // ID, images, text.
    public static final String ID = WrestlerMod.makeID("BottledWrestlerRelic");
    private static final Texture IMG = TextureLoader.getTexture(makeRelicPath("BottledWrestler.png"));
    private static final Texture OUTLINE = TextureLoader.getTexture(makeRelicOutlinePath("BottledWrestler.png"));

    public BottledWrestlerRelic() {
    ...
    // I've included that already in patches.relics.BottledWrestlerField
    // The basemod wiki I linked above has comments about onSave and onLoad

    @Override
    public Predicate<AbstractCard> isOnCard() {
        return BottledWrestlerField.inBottledWrestlerField::get;
    }
    ...
    public void onLoad(Integer cardIndex) {
        ...
        if (cardIndex >= 0 && cardIndex < AbstractDungeon.player.masterDeck.group.size()) {
            ...
            if (card != null) {
                BottledWrestlerField.inBottledWrestlerField.set(card, true);
    ...
    public void onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the bottled card from our deck for example.)
            ...
            if (cardInDeck != null) {
                BottledWrestlerField.inBottledWrestlerField.set(cardInDeck, false); // In our SpireField - set the card to no longer be bottled. (Unbottle it)
            }
        }
    }
    ...
    public void update() {
        ...
        if (!cardSelected && !AbstractDungeon.gridSelectScreen.selectedCards.isEmpty()) {
            ...
            BottledWrestlerField.inBottledWrestlerField.set(card, true); // Use our custom spire field to set that card to be bottled.
    ...
    public void onUseCard(AbstractCard targetCard, UseCardAction useCardAction) { // Whenever we use any card
        ...
        for (Iterator<AbstractCard> it = AbstractDungeon.player.drawPile.group.iterator(); it.hasNext(); ) {
            ...
            if (BottledWrestlerField.inBottledWrestlerField.get(card)) { // Check if our SpireField matches said card
